...
trait HasRequestVehicleAttribute
{
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Maintenance Count Attribute
    |--------------------------------------------------------------------------
    */

    protected function maintenanceCount(): Attribute
    {
        return Attribute::make(
            get: fn() => $this->getRawOriginal('maintenance_count')
                ?? static::query()
                ->wherePlateNumber($this->getPlateNumber())
                ->count()
        );
    }
